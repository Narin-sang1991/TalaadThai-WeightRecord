...
{
    public class HwUnitOfWork : QueryableUnitOfWork
    {
        private IUnityContainer container;

        public HwUnitOfWork(string configurationName, IUnityContainer container)
        ...
        public override int SaveChanges()
        {
            ...
            UserProfile profile = this.container.Resolve<AuthenticationServiceBase>().GetCurrentUserProfile(false);//new UserProfile(HardwareInfo.GetComputerName(), Guid.Empty, null, null);
                ...
                if (item.Entity is HwEntity)
                {
                    ...
                    if (item.State == EntityState.Added)
                    {
                        if (profile != null && profile.UserId != Guid.Empty)
                            erpEntityItem.MustData.SetMustDataNewMode(profile.UserId.ToString(), null, DateTimeOffset.Now);
                        else
                    ...
                    {
                        if (profile != null && profile.UserId != Guid.Empty)
                            erpEntityItem.MustData.SetMustDataUpdateMode(profile.UserId.ToString(), null, DateTimeOffset.Now);
                        else
